...
export default defineEventHandler(async (event) => {
    ...
    // Get the contact person associated with this site
    const users = await db.select().from(tables.users).where(eq(tables.users.uuid, site.contactPersonUuid)).limit(1)

    if (!users || users.length === 0) {
        throw createError({
            statusCode: 404,
            statusMessage: 'Contact person not found for this site'
        })
    }
    ...
    const response: ServerResponse<User> = {
        ...
        statusCode: 200,
        statusText: 'OK',
        message: 'Contact person retrieved successfully for site',
        data: user
    }
    return response
})
