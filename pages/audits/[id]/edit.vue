<!-- pages/audits/[id]/edit.vue -->
<template>
  <div class="mx-auto px-4 py-8 container">
    <div class="bg-white shadow-sm p-6 border border-gray-200 rounded-lg">
      <h1 class="mb-6 font-bold text-gray-900 text-2xl">Edit Audit Entry</h1>
      ...
      <div 
        ...
        <p class="text-gray-500">Loading audit data...</p>
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  try {
    const audit = await auditStore.fetchById(auditId)
    if (audit) {
      form.value = {
        action: audit.action,
...
const handleSubmit = async () => {
  try {
    await auditStore.update(auditId, form.value)
